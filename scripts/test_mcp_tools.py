"""Phase 22 MCP ツール呼び出しインタラクティブテスト。

web_search（Tavily 実装）を含む全ツールを呼び出して応答を確認する。
...
オプション:
  --tool <name>   特定のツールのみ実行 (ping / web_search / db_query_stub / claude_code_stub)
...
TOOL_INPUTS: dict[str, dict] = {
    "ping": {},
    "web_search": {"query": "LangGraph MCP integration"},
...
def main() -> None:
    ...
    parser.add_argument("--tool", choices=["ping", "web_search", "db_query_stub", "claude_code_stub"], help="特定のツールのみ実行")
