...
- [ ] `ChatCopilot` — `BaseChatModel` を継承した LangChain 互換ラッパーを実装する
- [ ] Device Flow 認証 — GitHub OAuth でトークンを取得し、暗号化してローカルに保存・再利用する
...
|----------|-----------|---------|
| `BaseChatModel` ラッパー実装 | Copilot SDK は JSON-RPC 通信のため OpenAI 互換 URL では代替不可 | — Pending |
| Device Flow 認証 | ブラウザ経由で簡単に認証でき、トークン再利用でインタラクション最小化 | — Pending |
| モデル選択可能 | gpt-4.1 固定より、Copilot が提供する他モデルも試せる柔軟性を優先 | — Pending |
